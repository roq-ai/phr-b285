...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getInsuranceDetails() {
    ...
    const response = await prisma.insurance_detail
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'insurance_detail'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createInsuranceDetail() {
    await insuranceDetailValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.insurance_detail.create({
      data: body,
    });
