...
import { getLaboratoryResults } from 'apiSdk/laboratory-results';
...
function DashboardPage() {
  ...
  const laboratoryResultStat = useSWR<PaginatedInterface<LaboratoryResultInterface>>(
    () => `/laboratory-results`,
    () => getLaboratoryResults({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: laboratoryResultStat?.data?.totalCount,
      isLoading: laboratoryResultStat.isLoading,
    },
  ];
